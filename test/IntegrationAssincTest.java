...
public class IntegrationAssincTest extends WithServer {
	...
	@Before
	public void setUp() {
		Logger.debug("************ RUNNING INTEGRATION ASSINC TESTS ************");
	...
	private void InitTest(String map, String origin, String destiny, String routeExpected, String costExpected, String distancyExpected) {
		...
		String body = RestAssured.given().contentType(ContentType.TEXT).content(map).expect().statusCode(200).when().post("/process/assinc/BH").body().asString();
		...
		StatusMap status;
		try {
			status = checkProtocol(protocol);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		assertThat(status.getStatus().name(), containsString("PROCESSED"));

		Logger.debug("************ VERIFY BEST ROUTE " + origin + " TO " + destiny + " ************");
		String best = RestAssured.expect().statusCode(200).when().get("/best/BH/" + origin + "/" + destiny + "/10").body().asString();
	...
	private StatusMap checkProtocol(ProtocolResponse protocol) {
		StatusMap status;
		Logger.debug("************ CHECK PROTOCOL ************");
		do {
			String check = RestAssured.expect().statusCode(200).when().get("/check/" + protocol.getName()).body().asString();

			JsonNode statusJson = Json.parse(check);
			status = Json.fromJson(statusJson, StatusMap.class);
			Logger.debug(String.format("Status: %s", status.getStatus().name()));
			
			if(status.getStatus().equals(StatusEnum.IN_PROCESS)){
				try {
					Thread.sleep(3000);
		...
		} while (status.getStatus().equals(StatusEnum.IN_PROCESS));
		return status;
	}
